...
        icon: 'error',
        title: 'Gagal Memuat',
        text: 'Gagal memuat daftar cerita. Anda harus login.', 
    });
    return []; 
  }
};
...
  const token = localStorage.getItem('authToken');
  if (!token) {
    console.error('Tidak ada token, tidak bisa menyimpan subscription.');
    return; 
  }
...
  try {
    const response = await fetch(API_CONFIG.PUSH_SUBSCRIBE, {
      method: 'POST',
      headers: headers, 
...
  } catch (error) {
    console.error('Error savePushSubscription:', error.message);
  }
};
